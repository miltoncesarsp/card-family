...
// NOVO: Função para exibir notificações (melhor UX que alert)
...
// NOVO: Função para deletar a imagem antiga
async function deleteOldImage(imageUrl) {
    if (!imageUrl || imageUrl.includes('anon.supabase.co')) return; // Evita deletar URLs não Supabase ou vazias

    try {
        // Extrai o caminho do arquivo (filePath) da URL pública do Supabase
        const urlParts = imageUrl.split(BUCKET_NAME + '/');
        if (urlParts.length < 2) {
            console.warn("URL da imagem antiga inválida:", imageUrl);
            return;
        }
...
            // Não retorna, pois o erro de Storage não deve impedir a atualização da carta
        } else {
            console.log("Imagem antiga deletada com sucesso:", filePath);
        }
    } catch (e) {
        console.error("Exceção ao tentar deletar imagem antiga:", e);
    }
}
...
    if (error || !cardData) { console.error("Erro ao buscar carta para edição:", error); showNotification("Erro ao carregar dados da carta.", true); return; }
...
    document.getElementById("currentImageUrl").value = cardData.image_url; // Guarda a URL para edição
...
    // Em admin.js, o elemento é 'Terra' para o preview, já que o elemento real vem do Personagem Base no save.
    const element = "Terra"; 
...
    container.innerHTML = "";
    
    // Usa a imagem existente se estiver editando e não houver um novo arquivo
    const finalImageUrl = file ? URL.createObjectURL(file) : (imageUrl || document.getElementById("currentImageUrl").value);

    if (!name && !power && !finalImageUrl) return;
...
    let existingImageUrl = document.getElementById("currentImageUrl").value;

    if (!name || !rarity || isNaN(power) || power < 0) { showNotification("Preencha Nome, Raridade e Força (Força deve ser um número positivo)!", true); return; }
    if (!isEditing && !file) { showNotification("Selecione uma imagem para a nova carta!", true); return; }

    // 1. Busca Personagem Base
...
        showNotification("Não foi possível encontrar o Personagem Base! Crie-o primeiro.", true);
        return;
    }
...
    // 2. Lógica de Upload da imagem e DELEÇÃO da antiga
    if (file) {
...
            showNotification(`Erro ao enviar a imagem: ${uploadError.message}.`, true); 
            return; 
        }
...
        // NOVO: Deleta a imagem antiga após o upload bem-sucedido
        if (isEditing && existingImageUrl && existingImageUrl !== imageUrlToSave) {
            await deleteOldImage(existingImageUrl);
        }
    } else if (!isEditing || !existingImageUrl) {
        // Previne salvar se não é edição e não tem imagem OU é edição e a imagem foi perdida
        showNotification("Carta sem imagem. Por favor, faça upload de uma.", true);
        return;
    }
    
    // 3. Monta e Salva
...
    if (dbError) { console.error("Erro ao salvar/atualizar no banco:", dbError); showNotification("Erro ao salvar/atualizar no banco! (Verifique RLS e colunas)", true); return; }

    showNotification(`Carta ${isEditing ? 'atualizada' : 'salva'} com sucesso!`);
    resetFormState();
...
    if (!personagem || !origem || !elemento) { showNotification("Preencha todos os campos do formulário Base!", true); return; }
...
    if (dbError) { console.error("Erro ao salvar Base:", dbError); showNotification(`Erro ao salvar no banco (Base): ${dbError.message}`, true); return; }

    showNotification(`Personagem Base "${personagem}" ${isEditingBase ? 'atualizado' : 'salvo'} com sucesso!`);
    resetBaseFormState();
    await loadUnifiedView();
}
...
        const { error, data } = await supabase.from('cards').select('image_url').eq('id', cardId).single();
        if (error || !data) { console.error("Erro ao buscar URL da carta para exclusão:", error); showNotification("Erro ao buscar dados da carta para exclusão.", true); return; }

        const imageUrl = data.image_url;
        
        const { error: deleteError } = await supabase.from('cards').delete().eq('id', cardId);
        if (deleteError) { 
            console.error("Erro ao deletar carta:", deleteError); 
            showNotification("Erro ao deletar carta. Verifique as permissões de RLS.", true); 
        } 
        else { 
            await deleteOldImage(imageUrl); // Deleta imagem do storage
            showNotification(`Carta ${cardName} deletada com sucesso!`); 
            await loadUnifiedView(); 
        }
    }
}
...
    if (error || !baseData) { console.error("Erro ao buscar Personagem Base para edição:", error); showNotification("Erro ao carregar dados do Personagem Base.", true); return; }
...
    document.getElementById("currentImageUrl").value = ""; // Limpa URL oculta
...
    const { data: cards, error: cardsError } = await supabase.from('cards').select('id, image_url').eq('id_base', baseId);
    if (cardsError) { console.error("Erro ao verificar cartas ligadas:", cardsError); showNotification("Erro ao verificar cartas ligadas.", true); return; }
...
        // Deleta cartas do banco
        const { error: deleteCardsError } = await supabase.from('cards').delete().eq('id_base', baseId);
        if (deleteCardsError) { console.error("Erro ao deletar cartas ligadas:", deleteCardsError); showNotification("Erro ao deletar cartas ligadas.", true); return; }
        
        // Deleta imagens do storage
        for (const card of cards) { await deleteOldImage(card.image_url); }
    }

    const { error: deleteBaseError } = await supabase.from('personagens_base').delete().eq('id_base', baseId);

    if (deleteBaseError) { console.error("Erro ao deletar Personagem Base:", deleteBaseError); showNotification("Erro ao deletar Personagem Base.", true); } 
    else { showNotification(`Personagem Base "${baseName}" e suas cartas ligadas deletados com sucesso!`); await loadUnifiedView(); }
}
...
        .single();
        
    if (error || !rule) { showNotification("Erro ao carregar regra de raridade.", true); return; }
    
...
    if (!nome || isNaN(custo) || isNaN(ordem)) { showNotification("Preencha todos os campos corretamente!", true); return; }
...
        dbError = error;
    } else {
        showNotification("Erro: O formulário deve ser usado apenas para EDITAR raridades existentes. Use o botão 'Editar' na tabela.", true);
        return;
    }

    if (dbError) { console.error("Erro ao salvar regra:", dbError); showNotification(`Erro ao salvar no banco: ${dbError.message}`, true); return; }

    showNotification(`Regra de raridade "${nome}" atualizada com sucesso!`);
...
    
    // Oculta o formulário de Pacote
    const formContainer = document.getElementById('packFormContainer');
    if (formContainer) {
...
    if (error || !pack) { showNotification("Erro ao carregar dados do pacote.", true); return; }
...
    
    // Multiplica por 100 para exibir em percentual (com 1 casa decimal)
...
    if (!nome || isNaN(preco) || preco < 0 || isNaN(totalCartas) || totalCartas <= 0 || isNaN(chanceComum)) {
        showNotification("Preencha todos os campos do Pacote corretamente!", true);
        return;
    }
    
    const totalChance = chanceComum + chanceRara + chanceEpica + chanceLendaria + chanceMitica;
    if (Math.abs(totalChance - 1.00) > 0.001) { 
        showNotification(`A soma das chances deve ser 100%! Sua soma atual é ${(totalChance * 100).toFixed(2)}%. Ajuste os valores.`, true);
        return;
    }
...
    if (dbError) { console.error("Erro ao salvar Pacote:", dbError); showNotification(`Erro ao salvar no banco: ${dbError.message}`, true); return; }

    showNotification(`Pacote "${nome}" ${isEditing ? 'atualizado' : 'criado'} com sucesso!`);
    
    // Esconde o formulário após salvar
    const formContainer = document.getElementById('packFormContainer');
    if (formContainer) {
...
        showNotification("Erro ao deletar pacote.", true); 
    } 
    else { 
        showNotification(`Pacote "${name}" deletado com sucesso!`); 
        await loadPacks(); 
    }
}
...
    if (error || !player) { showNotification("Erro ao carregar dados do jogador.", true); return; }
...
    if (!id || isNaN(moedas) || moedas < 0 || isNaN(nivel) || nivel < 1) {
        showNotification("Dados inválidos para edição de jogador (Moedas >= 0, Nível >= 1).", true);
        return;
    }
...
        showNotification(`Erro ao atualizar jogador "${nome}": ${error.message}`, true);
    } else {
        showNotification(`Jogador "${nome}" atualizado com sucesso!`);
        
        const formSection = document.getElementById('playerEditFormSection');
        if (formSection) {
            formSection.style.display = 'none';
...
        showNotification(`Erro ao deletar jogador "${name}". Verifique restrições de chaves estrangeiras (RLS/Foreign Keys).`, true);
    } else {
        showNotification(`Jogador "${name}" deletado com sucesso!`);
        await loadPlayers();
    }
}
